...
if (isMainThread) {
    let
        ha = {  // shouldn't need to touch anything below this line except for log() function (to add your automation name)
            fetch: function (client) {
                ...
                function finished() {
                    ...
                    if (client.ha.length == completed) {
                        // log("F" + JSON.stringify(buf) + "  client port: " + client.port)
                        udp.send(JSON.stringify({ type: "haFetchReply", obj: buf }), client.port)
                    }
                }
            },
            ws: function (reconnect) {
                if (reconnect == true) { wsClient.connect("ws://" + cfg.homeAssistant.address + ":" + cfg.homeAssistant.port + "/api/websocket"); return; }
                let ws = state.ha.ws;
                wsClient.connect("ws://" + cfg.homeAssistant.address + ":" + cfg.homeAssistant.port + "/api/websocket");
                ...
                wsClient.on('connect', function (socket) {
                    ...
                    socket.on('message', function (message) {
                        ...
                        switch (buf.type) {
                            case "pong":
                                ...
                                if (timeResult > 100) log("websocket ping is lagging - delay is: " + timeResult + "ms", 1, 2);
                                break;
                            case "auth_required":
                                log("Websocket authenticating", 1);
                                send({ type: "auth", access_token: cfg.homeAssistant.token, });
                                break;
                            case "auth_ok":
                            ...
                            case "event":
                                switch (buf.event.event_type) {
                                    case "state_changed":
                                        if (cfg.homeAssistant.legacyAPI == false && state.perf.ha.wait == true) {
                                            let delay = ha.perf();
                                            ...
                                            for (let y = 0; y < state.udp[x].ha.length; y++) {
                                                if (state.udp[x].ha[y] == buf.event.data.entity_id) {
                                                    ...
                                                    if (ibuf === "on") obuf = true;
                                                        ...
                                                        if (cfg.homeAssistant.log.espDisconnect == true)
                    ...
                    function ping() {
                        if (ws.reply == false) {
                            if (ws.pingsLost <= 0) {
                                log("websocket ping never got replied", 1, 3);
                                ws.pingsLost++;
                            }
                            else {
                                ws.online = false;
            ...
            perf: function () {
                state.perf.ha.wait = false;
                let delay = Date.now() - state.perf.ha.start;
                if (delay < state.perf.ha.best) state.perf.ha.best = delay;
                if (delay > state.perf.ha.worst) state.perf.ha.worst = delay;
                let total = 0;
                if (state.perf.ha.last100Pos < 99) {
                    if (state.perf.ha.last100[state.perf.ha.last100Pos]) state.perf.ha.last100[state.perf.ha.last100Pos++] = delay;
                    else state.perf.ha.last100.push(delay);
                }
                else {
                    if (state.perf.ha.last100[state.perf.ha.last100Pos]) state.perf.ha.last100[state.perf.ha.last100Pos] = delay;
                    else state.perf.ha.last100.push(delay);
                    state.perf.ha.last100Pos = 0;
                    state.perf.ha.worst = 0;
                    state.perf.ha.best = 1000;
                }
                state.perf.ha.last100.forEach(element => { total += element });
                state.perf.ha.average = Math.floor(total / state.perf.ha.last100.length);
                return delay;
            }
        },
        sys = {
            boot: function (step) {
                switch (step) {
                    ...
                    case 3:     // connect to Home Assistant
                        if (cfg.homeAssistant.enable == true) {
                            haconnect();
                            function haconnect() {
                                hass.status()
                                    .then(data => {
                                        log("HA push service " + a.color("green", data.message), 1);
            ...
            lib: function () {
                a = {
                    ...
                    hass = new HomeAssistant({
                        host: "http://" + cfg.homeAssistant.address,
            ...
            udp: function (data, info) {
                ...
                switch (buf.type) {
                    ...
                    case "haQuery":     // HA device list request
                        hass.states.list()
                            .then(dbuf => {
                                log("sending all HA entities to UDP", 3, 1);
                                dbuf.forEach(element => {
                                    udp.send(JSON.stringify({ type: "haQueryReply", obj: element.entity_id }), port);
                                });
                            })
                            .catch(err => {
                    ...
                    case "haState":     // incoming state change (Home Assistant)
                        state.perf.ha.start = Date.now();
                        state.perf.ha.wait = true;
                        state.perf.ha.id = state.ha.ws.id;
                        let sort = ["input_boolean", "input_button", "switch"];
                        if (buf.obj.unit == undefined) {
                            //     log("sensor test, name: " + buf.obj.name + " value: " + buf.obj.state, 0, 0);
                            for (let x = 0; x < sort.length; x++) {
                                if (buf.obj.name) {
                                    if (buf.obj.name.includes(sort[x])) {
                                        if (cfg.homeAssistant.legacyAPI == false) {
                                            em.emit('send', {
                                                "id": state.ha.ws.id++,
                                                "type": "call_service",
                                                "domain": sort[x],
                                            ...
                                            hass.services.call(buf.obj.state == false ? 'turn_off' : 'turn_on', sort[x], { entity_id: buf.obj.name })
                                                .then(dbuf => {
                                                    let delay = ha.perf();
                        ...
                        if (buf.obj.unit != undefined && buf.obj.name != undefined) {
                            ...
                            hass.states.update('sensor', "sensor." + buf.obj.name,
                    ...
                    case "register":    // incoming registrations
                        log("New client Reg, address: " + info.address + ":" + info.port + " ID: " + id + " Name: " + state.udp[id].name, 3, 1);
                        if (buf.obj.ha != undefined) {
                            log("client " + id + " - " + info.address + " is registering Home Assistant entities", 3, 1);
                        ...
                        if (buf.obj.esp != undefined) {
                            ...
                            log("client " + id + " - " + info.address + " is registering ESP entities", 3, 1);
                        }
                        if (buf.obj.telegram != undefined && buf.obj.telegram == true) {
                            log("client " + id + " - " + info.address + " is registering as telegram agent", 3, 1);
                ...
                function checkUDPreg() {
                    ...
                    if (registered == false) {
                        ...
                        if (buf.type != "register") {
                            log("out of sync client, requesting rereg, bytes: " + data.length
                                + " address: " + info.address + ":" + info.port + " ID: " + id + " time: " + time, 3, 2);
            ...
            log: function (message, mod, level, port) {      // add a new case with the name of your automation function
                ...
                if (cfg.telegram.enable == true && state.telegram.started == true) {
                    if (level >= cfg.telegram.logLevel
                        || cfg.telegram.logLevel == 0 && cfg.telegram.logDebug == true) {
